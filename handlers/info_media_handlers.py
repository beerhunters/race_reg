...
from .utils import logger, messages, config, RegistrationForm, get_participation_fee_text
...
def register_info_media_handlers(dp: Dispatcher, bot: Bot, admin_id: int):
        ...
        try:
            if os.path.exists(afisha_path):
                await bot.send_photo(
                    ...
                    caption=messages["info_message"].format(fee=get_participation_fee_text()),
                )
                logger.info(
                    f"Афиша отправлена с текстом info_message пользователю user_id={message.from_user.id}"
                )
            else:
                await message.answer(messages["info_message"].format(fee=get_participation_fee_text()))
                logger.info(
    ...
    async def process_afisha_image(message: Message, state: FSMContext):
        ...
        try:
            ...
            await message.answer(text)
            logger.info(f"Изображение афиши сохранено в {afisha_path}, размер: {final_size_kb:.1f} КБ")
            
        except Exception as e:
            logger.error(f"Ошибка при сохранении афиши от user_id={message.from_user.id}: {e}")
            await message.answer("❌ Ошибка при сохранении афиши. Проверьте, что отправляете изображение в правильном формате.")
    ...
    async def update_sponsor(event: [Message, CallbackQuery], state: FSMContext):
        ...
        sponsor_path = config.get("sponsor_image_path", "/app/images/sponsor_image.jpeg")
    ...
    async def process_sponsor_image(message: Message, state: FSMContext):
        ...
        try:
            ...
            await message.answer(text)
            logger.info(f"Изображение спонсоров сохранено в {sponsor_path}, размер: {final_size_kb:.1f} КБ")
            
        except Exception as e:
            logger.error(f"Ошибка при сохранении изображения спонсоров от user_id={message.from_user.id}: {e}")
            await message.answer("❌ Ошибка при сохранении изображения спонсоров. Проверьте формат файла.")
    ...
    async def update_info_message(event: [Message, CallbackQuery], state: FSMContext):
        ...
        if user_id != admin_id:
            await event.answer("❌ Доступ запрещен")
            return
        logger.info(f"Команда обновления информационного сообщения от user_id={user_id}")
