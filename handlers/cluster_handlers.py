...
def register_cluster_handlers(dp: Dispatcher, bot: Bot, admin_id: int):
    ...
    async def start_category_assignment(callback_query: CallbackQuery, state: FSMContext):
    ...
    async def start_cluster_assignment(callback_query: CallbackQuery, state: FSMContext):
    ...
    async def show_participant_for_assignment(message: Message, state: FSMContext, bot: Bot):
    ...
    async def process_category_selection(callback_query: CallbackQuery, state: FSMContext):
        ...
        if selected_category:
            ...
            if not success:
                await callback_query.message.answer(f"❌ Ошибка при сохранении категории для участника")
                return
    ...
    async def process_cluster_selection(callback_query: CallbackQuery, state: FSMContext):
        ...
        if selected_cluster:
            ...
            if not success:
                await callback_query.message.answer(f"❌ Ошибка при сохранении кластера для участника")
                return
    ...
    async def show_assignment_summary(message: Message, state: FSMContext, bot: Bot, assignment_type: str):
    ...
    async def clear_categories(callback_query: CallbackQuery):
        ...
        if success:
            ...
            text += "📝 Вы можете заново назначить категории через 'Добавить категории'."
        else:
    ...
    async def clear_clusters(callback_query: CallbackQuery):
        ...
        if success:
            ...
            text += "🎯 Вы можете заново назначить кластеры через 'Распределить кластеры'."
        else:
        ...
        for participant in participants:
            ...
            try:
                ...
                if category:
                    ...
                    msg_text += f"📂 <b>Ваша категория:</b> {category_emoji} {category}\n"
    ...
    async def create_document(callback_query: CallbackQuery):
        ...
        if not participants:
            await callback_query.message.answer("❌ Нет участников для создания документа")
            return
            ...
            for cat_name in category_order:
                ...
                if has_clusters:
                    ...
                    for cluster_name in cluster_order:
                        ...
                        for i, p in enumerate(sorted(cluster_participants, key=lambda x: x[2]), 1):
                            name = p[2]
                            target_time = p[3] or "—"
                            bib_number = p[7] if len(p) > 7 else None  # Check if bib_number exists
                            bib_info = f" (№{bib_number})" if bib_number else ""
                            document_text += f"    {i}. {name}{bib_info} - {target_time}\n"
                else:
                    # Just list participants in category
                    for i, p in enumerate(sorted(participants_in_cat, key=lambda x: x[2]), 1):
                        name = p[2]
                        target_time = p[3] or "—"
        ...
        if len(document_text) > 4000:
            ...
            await callback_query.message.answer("📄 <b>Создаю документ для печати...</b>")
    ...
    async def download_csv(callback_query: CallbackQuery):
        ...
        try:
            ...
            sorted_participants = sorted(runners_only, key=lambda p: (
            ...
            # Create file
            filename = f"beer_mile_distribution_{current_time.strftime('%Y%m%d_%H%M%S')}.csv"
            ...
            logger.info(f"CSV файл распределения отправлен администратору, бегунов: {len(sorted_participants)}")
