...
def register_backup_handlers(dp: Dispatcher, bot: Bot, admin_id: int):
    ...
    async def create_manual_backup(callback_query: CallbackQuery):
        ...
        status_message = await callback_query.message.answer("💾 <b>Создание резервной копии...</b>")
        ...
        try:
            ...
            if backup_file and os.path.exists(backup_file):
                ...
                await status_message.edit_text("❌ <b>Ошибка при создании резервной копии</b>")
    ...
    async def backup_settings(callback_query: CallbackQuery):
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="💾 Создать бекап", callback_data="admin_create_backup"),
                ],
                [
                    InlineKeyboardButton(text="🔄 Автобекапы", callback_data="admin_toggle_auto_backup"),
                    InlineKeyboardButton(text="🧹 Очистить старые", callback_data="admin_cleanup_backups"),
                ],
                [
    ...
    async def restore_backup_menu(callback_query: CallbackQuery, state: FSMContext):
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="🔙 Назад", callback_data="admin_backup_settings"),
                ]
            ]
        )
    ...
    async def process_restore_backup(message: Message, state: FSMContext):
        ...
        status_message = await message.answer("📥 <b>Восстановление из резервной копии...</b>")
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Восстановление из резервной копии выполнено администратором: {message.document.file_name}")
            else:
                await status_message.edit_text(
        ...
        try:
            ...
            if backup_file and os.path.exists(backup_file):
                try:
                    ...
                    logger.error(f"Ошибка при отправке автоматической резервной копии: {e}")
    ...
    try:
        ...
        for config_file in ["config.json", "messages.json"]:
            ...
            if os.path.exists(config_backup_path):
                ...
                if os.path.exists(config_file):
                    ...
                    logger.info(f"Текущий файл {config_file} сохранен как {current_config_backup}")
                
                # Restore config
                shutil.copy2(config_backup_path, config_file)
                logger.info(f"Восстановлен файл конфигурации: {config_file}")
            else:
                logger.warning(f"Файл конфигурации {config_file} не найден в резервной копии")
        ...
        if os.path.exists(images_backup_dir):
            ...
            if os.path.exists(images_target_dir):
                ...
                logger.info(f"Текущая директория изображений сохранена в: {current_images_backup}")
