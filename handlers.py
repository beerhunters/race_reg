...
from aiogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, BufferedInputFile, FSInputFile
from database import add_participant, get_all_participants, get_participant_count, get_participant_by_user_id, update_payment_status, delete_participant, get_participant_count_by_role
...
def register_handlers(dp: Dispatcher, bot: Bot, admin_id: int):
    ...
    async def cmd_start(message: Message, state: FSMContext):
        ...
        if message.from_user.id == admin_id:
            logger.info(f"Пользователь user_id={message.from_user.id} является администратором")
            await message.answer(messages['admin_commands'], parse_mode=None)
            await state.clear()
            return
        participant = get_participant_by_user_id(message.from_user.id)
        if participant:
            logger.info(f"Пользователь user_id={message.from_user.id} уже зарегистрирован")
            name = participant[2]
            target_time = participant[3]
            role = participant[4]
            time_field = f"Целевое время: {target_time}" if role == 'runner' else "Вы волонтер"
            await message.answer(messages['already_registered'].format(name=name, time_field=time_field, role=role))
            await state.clear()
            return
    ...
    async def process_role(callback_query, state: FSMContext):
        ...
        if role == 'runner':
            ...
            success = add_participant(callback_query.from_user.id, username, name, target_time, role)
            if success:
                logger.info(f"Успешная регистрация: {name}, {role}, user_id={callback_query.from_user.id}")
                time_field = "Вы волонтер"
                extra_info = ""  # Нет текста про оплату для волонтеров
                user_message = messages['registration_success'].format(name=name, time_field=time_field, extra_info extra_info)
                ...
                try:
                    image_path = '/app/images/sponsor_image.jpg'
                    if os.path.exists(image_path):
                        ...
                        logger.info(f"Сообщение со спонсорами отправлено пользователю user_id={callback_query.from_user.id}")
                    else:
                        logger.warning(f"Файл {image_path} не найден, отправляется только текст спонсоров")
                        await callback_query.message.answer(messages['sponsor_message'])
                except Exception as e:
                    logger.error(f"Ошибка при отправке сообщения со спонсорами пользователю user_id={callback_query.from_user.id}: {e}")
                    await callback_query.message.answer(messages['sponsor_message'])
                logger.info(f"Сообщения отправлены: пользователю и админу (admin_id={admin_id})")
                participant_count = get_participant_count()
                logger.info(f"Всего участников: {participant_count}")
            else:
                logger.error(f"Ошибка регистрации для user_id={callback_query.from_user.id}")
                await callback_query.message.answer("Ошибка при регистрации. Попробуйте снова.")
            await state.clear()
    ...
    async def process_target_time(message: Message, state: FSMContext):
        ...
        logger.info(f"Получено целевое время: {target_time} от user_id={message.from_user.id}")
        user_data = await state.get_data()
        name = user_data.get('name')
        role = user_data.get('role')
        username = message.from_user.username or "не указан"
        success = add_participant(message.from_user.id, username, name, target_time, role)
        if success:
            logger.info(f"Успешная регистрация: {name}, {role}, user_id={message.from_user.id}")
            ...
            try:
                ...
                if os.path.exists(image_path):
                    ...
                    logger.info(f"Сообщение со спонсорами отправлено пользователю user_id={message.from_user.id}")
                else:
                    logger.warning(f"Файл {image_path} не найден, отправляется только текст спонсоров")
                    await message.answer(messages['sponsor_message'])
            except Exception as e:
                logger.error(f"Ошибка при отправке сообщения со спонсорами пользователю user_id={message.from_user.id}: {e}")
                await message.answer(messages['sponsor_message'])
            logger.info(f"Сообщения отправлены: пользователю и админу (admin_id={admin_id})")
            participant_count = get_participant_count()
            logger.info(f"Всего участников: {participant_count}")
    ...
    async def show_participants(message: Message):
        ...
        for index, (user_id, username, name, target_time, role, reg_date, payment_status) in enumerate(participants, 1):
    ...
    async def show_stats(message: Message):
        ...
        with sqlite3.connect('/app/data/race_participants.db') as conn:
            ...
            cursor.execute('SELECT COUNT(*) FROM participants WHERE DATE(reg_date) = DATE("now")')
            today_count = cursor.fetchone()[0]
        stats_message = messages['stats_message'].format(total=total_count, paid=paid_count, pending=pending_count, today=today_count)
    ...
    async def mark_as_paid(message: Message):
        ...
        if participant:
            ...
            try:
                await bot.send_message(chat_id=user_id, text=messages['payment_confirmed'])
    ...
    async def export_participants(message: Message):
        ...
        writer.writerow(["Имя", "Целевое время", "Роль", "Дата регистрации", "Статус оплаты", "Username"])
        for user_id, username, name, target_time, role, reg_date, payment_status in participants:
            writer.writerow([name, target_time, role, reg_date, payment_status, username])
        ...
        await message.answer_document(document=BufferedInputFile(csv_bytes, filename="participants.csv"))
        logger.info(f"CSV-файл успешно отправлен для user_id={message.from_user.id}")

    @dp.message()
    async def handle_other_messages(message: Message):
        logger.info(f"Неизвестная команда от user_id={message.from_user.id}: {message.text}")
